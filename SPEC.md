...
- Each **internal node** is a split — either horizontal (top / bottom, horizontal divider) or vertical (side by side, vertical divider)
- Any pane can be split further into two via a UI widget or using a keyboard shortcut
- There is no artificial limit on nesting depth — a split pane can itself be split recursively. The only practical constraint is available canvas size and minimum usable element dimensions.
